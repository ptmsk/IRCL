...
if __name__ == "__main__":
    ...
    for i, path in enumerate(sorted(img_paths)):
        ...
        if args.save_vis:
            ...
            label_image, elapsed_time = ircl.vis_run_ccl(image, args.save_vis)
            play_vis(args.save_vis, "video")
        else:
            label_image, elapsed_time = ircl.run_ccl(image)

            color_code = gen_color_code(list(set(ircl.rtable)))
